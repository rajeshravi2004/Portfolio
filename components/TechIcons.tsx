...
function Icon({ children }: { children: ReactNode; label: string }) {
  return <svg className="tech-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">{children}</svg>;
}
...
export function TechIcon({ name }: { name: string }) {
  ...
  if (key.includes("postgres") || key.includes("faiss") || key.includes("mysql") || key.includes("mongo") || key.includes("oracle") || key.includes("data processing")) return <Icon label={name}><ellipse cx="12" cy="5" rx="7" ry="3" /><path d="M5 5v7c0 1.7 3.1 3 7 3s7-1.3 7-3V5M5 12v6c0 1.7 3.1 3 7 3s7-1.3 7-3v-6" /></Icon>;
  ...
  if (key.includes("llm") || key.includes("prompt") || key.includes("gemini") || key.includes("langchain") || key.includes("chat") || key.includes("ai / ml") || key.includes("deep learning")) return <Icon label={name}><path d="M12 2l1.8 6.2L20 10l-6.2 1.8L12 18l-1.8-6.2L4 10l6.2-1.8L12 2z" /><path d="M19 15l.7 2.3L22 18l-2.3.7L19 21l-.7-2.3L16 18l2.3-.7L19 15z" /></Icon>;
  ...
  if (key.includes("youtube")) return <Icon label={name}><rect x="2" y="6" width="20" height="12" rx="4" /><path d="M10 9l5 3-5 3V9z" fill="currentColor" /></Icon>;
  if (key === "stripe") return <Icon label={name}><rect x="3" y="3" width="18" height="18" rx="4" fill="currentColor" /><path d="M15.5 8.2c-1-.5-2-.7-3.2-.7-1.8 0-3 .8-3 2.1 0 2.3 6 1.2 6 4.4 0 1.5-1.3 2.5-3.4 2.5-1.3 0-2.5-.3-3.4-.8" stroke="var(--bg)" /></Icon>;
  ...
  if (key.includes("word") || key.includes("excel") || key.includes("powerpoint") || key.includes("tally")) {
    ...
    return <Icon label={name}><rect x="3" y="3" width="18" height="18" rx="2" fill="currentColor" /><text x="7" y="17" className="tech-icon-text tech-icon-text-dark">{letter}</text></Icon>;
  }
